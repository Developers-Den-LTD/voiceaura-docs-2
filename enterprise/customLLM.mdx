...
Voiceaura will fine-tune a custom model for your enterprise using transcripts from succesful prior calls. Then Voiceaura will host that LLM and provided dedicated infrastrucure to enable phone conversations with sub-second latency.

Voiceaura will also enable you to connect to a custom LLM & will host that LLM to drive latency down further.

Get in touch [here](https://www.voiceaura.ai/survey/).
...
## How do I fine-tune with Voiceaura?

To inquire about fine-tuning connect with the Voiceaura team [here](https://www.voiceaura.ai/survey/).
...
The Voiceaura team will advise on connection method, requirements for the connection, etc.

Typically takes under 24 hours to set up after kickoff.

Inquire [here](https://www.voiceaura.ai/survey/)