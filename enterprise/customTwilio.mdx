...
description: "Connect Voiceaura to your own Twilio account"
---

## Overview

Enterprise customers can connect their own Twilio account to Voiceaura. Easily bring over your existing phone numbers, integrations, and more.
...
2. create an  ```encrypted_key``` by [sending an API request](http://docs.voiceaura.ai/api-reference/twilioAccounts/create) to Voiceaura.
...
- You can set your ```from``` number in the API request - this will need to be a number owned by that Twilio account (and not one purchased through Voiceaura).
...
2. Upload your numbers [through the API](http://docs.voiceaura.ai/api-reference/twilioAccounts/upload) 

We’ll validate that these numbers are owned by that account and add them to your Voiceaura account.
...
Once you update an inbound number through the [Dev Portal](https://api.platform.com/) or [API](http://docs.voiceaura.ai/api-reference/inbound/update), that number will be automatically configured to run on voiceaura’s infrastructure. No additional steps are required!
